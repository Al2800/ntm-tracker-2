# NTM Tracker - Agent Instructions
...
### Tech Stack
- **WSL Daemon:** Bun + TypeScript
- **Database:** SQLite (bun:sqlite)
- **Windows App:** Tauri 2.0 (Rust backend)
- **UI:** Svelte 5 + Tailwind
- **IPC:** HTTP + WebSocket over localhost

### Core Features
1. **Session & Pane Tracking** - Live status, idle/active/waiting
2. **Compact Detection** - Pattern matching + context-size drops
3. **Escalation Detection** - Human input/error prompt detection
4. **Usage Analytics** - Hourly/daily aggregates, token estimates
5. **System Tray Workflow** - Quick status, per-session actions
6. **Dashboard UI** - Overview cards, activity graph, escalation list

### Key Data Models
- `Session` - NTM session metadata and lifecycle
- `Pane` - Pane status, agent type, activity stats
- `CompactEvent` - Detected context compacts/resets
- `Escalation` - Attention-required prompts/errors
- `PaneSnapshot` - Periodic time-series samples
- `HourlyStats` / `DailyStats` - Aggregated usage

### Architecture Layers
```
Presentation  → Svelte UI (Dashboard + Tray)
Backend       → Tauri (Rust) app shell + daemon manager
Service       → Bun daemon (collector, detector, API/WS)
Data          → SQLite + in-memory cache
```

### Important Files (to be created)
- `daemon/src/index.ts` - Service entry point
- `daemon/src/collector.ts` - NTM/tmux polling + hook ingest
- `daemon/src/detector.ts` - Compact/escalation/status detection
- `daemon/src/db.ts` - SQLite schema + migrations
- `daemon/src/api.ts` - HTTP/WS server
- `app/src/*` - Tauri/Svelte UI
