...
<div class="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-5">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="mt-1 text-xs text-slate-500">Recent session, pane, and escalation events.</p>
    </div>
    <button
      type="button"
  ...
  {:else}
    <div class="mt-4 space-y-2 text-sm text-slate-300">
      {#each sorted as event (event.id)}
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800/80 bg-slate-950/70 px-3 py-2">
          <div class="flex flex-wrap items-center gap-3">
            <span class="text-xs text-slate-500">{formatTime(event.detectedAt)}</span>
            <span class="rounded-full border border-slate-700/80 px-2 py-0.5 text-xs text-slate-200">
              {event.type}
            </span>
